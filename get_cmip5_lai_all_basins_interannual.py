...
path = '/nfs/a68/gyjcab/datasets/cmip5/lai/historical/ensemble_means/'
startyr = 1994
endyr = 2004
...
for basin in basin_shapes:
    ...
    for model in models:
        ...
        print(model, 'MAX', np.nanmax(cube.data))
        print(model, 'MEAN', np.nanmean(cube.data))
        ...
        # adjust units
        if model == 'inmcm4':
            cube = cube/100  # units in %
            print(cube.data.max())

        if model == 'CMCC-CESM':
            # this model has 2D lat and lon
            lats = lats[:, 0]
            lons = lons[0, :]
        ...
        if basin == 'purus_dissolved':
            plt.figure()
            plt.imshow(cube.data[0, :, :], origin='lower')
            plt.colorbar()
            plt.savefig('/nfs/see-fs-02_users/earjba/public_html/et_' + model + '_test_lai.png')
...
np.save(outpath + 'cmip5_all_basins_lai_'+ str(startyr) + '_' +
