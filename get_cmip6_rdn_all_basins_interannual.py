...
def get_interannual_rdn(path, fname, constraint=None):
    cube = iris.load_cube(path+fname, constraint=constraint)
    #dates = get_dates(cube)
    #iris.coord_categorisation.add_month(cube, 'time', name='month')
    #data_by_month = cube.aggregated_by(['month'], iris.analysis.MEAN)
    #print(data_by_month.shape)
    return(cube)
...
path = '/nfs/a68/gyjcab/datasets/cmip6/rsds/historical/ensemble_means/'
startyr = 2003
endyr = 2013
...
for basin in basin_shapes:
    ...
    for model in models:
        ...
        if basin == 'solimoes_dissolved5':
            if model == 'NorESM1-ME':
                plt.figure()
                plt.imshow(cube.data[0, :, :], origin='lower')
                plt.colorbar()
                plt.savefig('/nfs/see-fs-02_users/earjba/public_html/rdn_' + model + '_test.png')
...
np.save(outpath + 'cmip6_all_basins_rdn_'+ str(startyr) + '_' +
