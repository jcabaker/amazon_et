...
era5_et = iris.load_cube(path+'evspsbl*era5*deg*.nc', constraint=constraint)
era5_et.convert_units('kg m^-2 month^-1')
era5_dates = get_dates(era5_et)

# generate mask to extract data from basin
lats = get_lats(era5_et)
lons = get_lons(era5_et)
...
for basin in basin_shapes:
    ...
    for yr in range(era5_et.shape[0]):
        data = era5_et[yr, :, :]
    ...
    era5_df['basin_mean_et'] = era5_vals
...
np.save(outpath + 'reanalysis_all_basins_et_'+ str(startyr) + '_' +
